...
mod macho {
    ...
    pub(super) fn print_fat_arch<Arch: FatArch>(p: &mut Printer<impl Write>, arch: &Arch) {
        p.group("FatArch", |p| {
            ...
            p.field_hex("Offset", arch.offset());
            p.field_hex("Size", arch.size());
    ...
    fn print_segment<S: Segment>(
        ...
        p.group("SegmentCommand", |p| {
            ...
            p.field_hex("VmAddress", segment.vmaddr(endian));
            p.field_hex("VmSize", segment.vmsize(endian));
            p.field_hex("FileOffset", segment.fileoff(endian));
            p.field_hex("FileSize", segment.filesize(endian));
    ...
    fn print_section<S: Section>(
        ...
        p.group("Section", |p| {
            ...
            p.field_hex("Address", section.addr(endian));
            p.field_hex("Size", section.size(endian));
            ...
            if let Ok(symbols) = symtab.symbols::<Mach>(endian, data) {
                for (index, nlist) in symbols.iter().enumerate() {
                    p.group("Nlist", |p| {
                        ...
                        p.field_hex("Value", nlist.n_value(endian));
                    });
                }
            }
        });
