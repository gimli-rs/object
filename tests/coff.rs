...
fn coff_extended_relocations() {
    let path_to_obj: PathBuf = ["testfiles", "coff", "relocs_overflow.obj"]
        .iter()
        .collect();
    let contents = fs::read(&path_to_obj).expect("Could not read relocs_overflow.obj");
    let file =
        read::coff::CoffFile::parse(&contents[..]).expect("Could not parse relocs_overflow.obj");
    let code_section = file
        .section_by_name(".text")
        .expect("Could not find .text section in relocs_overflow.obj");
