...
impl<'data, R: ReadRef<'data>> WasmFile<'data, R> {
    ...
    pub fn parse(data: R) -> Result<Self> {
                    ...
                    if name == "name" {
                        ...
                        for subsection in wp::Dylink0SectionReader::new(reader) {
                            ...
                            match subsection {
                                ...
                                wp::Dylink0Subsection::ImportInfo(infos) => import_infos = infos,
                                wp::Dylink0Subsection::ExportInfo(infos) => export_infos = infos,
                                _ => {}
                            }
                        }
            ...
            for imports in imports_section {
                ...
                let add_import = &mut |module, ty, name| {
                    ...
                    file.imports.push(WasmImportInternal {
                        ...
                        kind,
                        index,
                        // Updated later using linking symbol or dylink info.
                        weak: false,
                    });
                };
                match imports {
        ...
        // Bias to apply to function indices when accessing `code_ranges`.
        ...
        if let Some(symbols) = symbols {
            // We have a symbol table, so we don't need to add symbols for names.
            names = None;
                ...
                let name = match symbol {
                    wp::SymbolInfo::Func {
                        ...
                        if let Some(local_func_index) = index.checked_sub(local_func_base) {
                            ...
                            let import = &mut file.imports[import_funcs[index as usize]];
                            import.weak = weak;
                    ...
                    wp::SymbolInfo::Global {
                        ...
                        if let Some(local_global_index) = index.checked_sub(local_global_base) {
                            ...
                            let import = &mut file.imports[import_globals[index as usize]];
                            import.weak = weak;
                            if !flags.contains(wp::SymbolFlags::EXPLICIT_NAME) {
                                name = Some(import.name);
                            }
                    ...
                    wp::SymbolInfo::Event {
                        ...
                        if let Some(import_index) = import_tags.get(index as usize) {
                            let import = &mut file.imports[*import_index];
                            import.weak = weak;
                            if !flags.contains(wp::SymbolFlags::EXPLICIT_NAME) {
                                name = Some(import.name);
                            }
        ...
        // Index into `file.symbols` for each local function.
        // Only needed for the scope of symbols from the `name` custom section.
        let mut local_func_symbols = Vec::new();

        if let Some(names) = names {
            local_func_symbols = vec![None; code_ranges.len()];
            for name in names {
                ...
                for naming in name_map {
                    ...
                    local_func_symbols[local_func_index as usize] = Some(file.symbols.len());
                    file.symbols.push(WasmSymbolInternal {
                        name: naming.name,
                        address,
                        size,
                        ...
                        // May be updated later using exports.
        ...
        if let Some(exports) = exports {
            for export in exports {
                ...
                let kind = match export.kind {
                    wp::ExternalKind::Func => wasm::WASM_EXTERNAL_FUNCTION,
                    wp::ExternalKind::Table => wasm::WASM_EXTERNAL_TABLE,
                    wp::ExternalKind::Memory => wasm::WASM_EXTERNAL_MEMORY,
                    wp::ExternalKind::Global => wasm::WASM_EXTERNAL_GLOBAL,
                    wp::ExternalKind::Tag => wasm::WASM_EXTERNAL_TAG,
                    wp::ExternalKind::FuncExact => wasm::WASM_EXTERNAL_FUNCTION_EXACT,
                };
                ...
                if export.kind == wp::ExternalKind::Func {
                    if let Some(local_func_index) = export.index.checked_sub(local_func_base) {
                        ...
                        if let Some(range) = code_ranges.get(local_func_index as usize) {
                            address = Some(range.0);
                        }

                        // Mark any matching symbol as exported.
                        if let Some(Some(symbol_index)) =
                            local_func_symbols.get(local_func_index as usize)
                        {
                            let symbol = &mut file.symbols[*symbol_index];
                            if symbol.name == export.name {
                                symbol.scope = SymbolScope::Dynamic;
                            }
                        }
                    }
                } else if export.kind == wp::ExternalKind::Global {
                    if let Some(local_global_index) = export.index.checked_sub(local_global_base) {
                        // Try to guess the linear memory address. Rust and C export a global
                        // containing the address in linear memory of the symbol.
                ...
                file.exports.push(WasmExportInternal {
                    ...
                    index: export.index,
                    address,
                    // Updated later using dylink info.
                    weak: false,
                });
            }
        }
        ...
        for info in import_infos {
            if info.flags.contains(wp::SymbolFlags::BINDING_WEAK) {
                for import in &mut file.imports {
                    if import.module == info.module && import.name == info.field {
                        import.weak = true;
                    }
                }
            }
        ...
        for info in export_infos {
            if info.flags.contains(wp::SymbolFlags::BINDING_WEAK) {
                for export in &mut file.exports {
                    if export.name == info.name {
                        export.weak = true;
                    }
                }
            }
...
impl<'data, 'file, R> Iterator for WasmExportIterator<'data, 'file, R> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let target = match export.address {
            Some(address) => ExportTarget::Address { address },
            None => ExportTarget::Wasm,
        };
        Some(Ok(Export {
