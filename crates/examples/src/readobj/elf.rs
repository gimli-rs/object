...
fn print_notes<Elf: FileHeader>(
    ...
    while let Some(Some(note)) = notes.next().print_err(p) {
        p.group("Note", |p| {
            ...
            p.field_consts("Type", note.n_type(endian), elf::nt_names(name));
...
fn constants<Elf: FileHeader>(endian: Elf::Endian, elf: &Elf) -> &'static elf::Constants {
    elf::machine_constants(elf.e_machine(endian))
}
